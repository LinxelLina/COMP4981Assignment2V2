...
void socket_connect(int sockfd, struct settings *set)
{
    ...
    if(inet_ntop(set->ip_address.ss_family, set->ip_address.ss_family == AF_INET ? (void *)&(((struct sockaddr_in *)&set->ip_address)->sin_addr) : (void *)&(((struct sockaddr_in6 *)&set->ip_address)->sin6_addr), addr_str, sizeof(addr_str)) == NULL)
    {
        perror("inet_ntop");
        exit(EXIT_FAILURE);
    }
    ...
    net_port = htons(set->port);

    if(set->ip_address.ss_family == AF_INET)
    {
        struct sockaddr_in *ipv4_addr;

        ipv4_addr           = (struct sockaddr_in *)&set->ip_address;
        ipv4_addr->sin_port = net_port;
        addr_len            = sizeof(struct sockaddr_in);
    }
    else if(set->ip_address.ss_family == AF_INET6)
    {
        struct sockaddr_in6 *ipv6_addr;

        ipv6_addr            = (struct sockaddr_in6 *)&set->ip_address;
        ipv6_addr->sin6_port = net_port;
        addr_len             = sizeof(struct sockaddr_in6);
    }
    else
    {
        fprintf(stderr, "Invalid address family: %d\n", set->ip_address.ss_family);
        exit(EXIT_FAILURE);
    }

    if(connect(sockfd, (struct sockaddr *)&set->ip_address, addr_len) == -1)
    {
        const char *msg;
...
void run_client(int sockfd)
{
    ...
    read:
        ...
        memset(buffer, '\0', LEN);
        // putting here because i want this to start the next loop.
        printf("\nEnter something:\n");
    }
}
